    ...
    init() {
        ...
        $(document).on('click', '.apply-coupon-code', event => {
            ...
            $.ajax({
                ...
                success: res => {
                    if (!res.error) {
                        $('#main-checkout-product-info').load(window.location.href + '?applied_coupon=1 #main-checkout-product-info > *');
                    } else {
        ...
        $(document).on('click', '.remove-coupon-code', event => {
            ...
            $.ajax({
                ...
                success: res => {
                    if (!res.error) {
                        $('#main-checkout-product-info').load(window.location.href + ' #main-checkout-product-info > *');
                    } else {
                        $('.coupon-error-msg .text-danger').text(res.message);
                    }
                    _self.find('i').remove();
