class ShippingManagement {
    init() {
        ...
        $(document).on('keyup', '.input-sync-item', event => {
            ...
            $(event.currentTarget).closest('.input-shipping-sync-wrapper').find($(event.currentTarget).data('target')).text(Botble.numberFormat(parseFloat(number)));
        });
        ...
        $(document).on('keyup', '.shipping-price-district', event => {
            let _self = $(event.currentTarget);
            let base_price = _self.closest('.shipping-detail-information').find('.base-price-rule-item').val();
            if (!base_price || isNaN(base_price)) {
                base_price = 0;
            }

            let adjustment_price = _self.val();

            if (!adjustment_price || isNaN(adjustment_price)) {
                adjustment_price = 0;
            }
            _self.closest('tr').find('.support-shipping .rule-adjustment-price-item').text(Botble.numberFormat(parseFloat(base_price) + parseFloat(adjustment_price)));
        });

        $(document).on('keyup', '.base-price-rule-item', event => {
            let _self = $(event.currentTarget);
            let base_price = _self.val();
            ...
            $.each($(document).find('.support-shipping .rule-adjustment-price-item'), (index, item) => {
                let adjustment_price = $(item).closest('tr').find('.shipping-price-district').val();
                if (!adjustment_price || isNaN(adjustment_price)) {
                    adjustment_price = 0;
                }
                $(item).text(Botble.numberFormat(parseFloat(base_price) + parseFloat(adjustment_price)));
            });
        });
    }
}
