...
if (!function_exists('get_discount_description')) {
    ...
    function get_discount_description($discount)
    {
        switch ($discount->type_option) {
            case 'shipping':
                if ($discount->target) {
                    $description = __('Free shipping to') . ' <strong>' . $discount->target->name . '</strong>';
                } else {
            ...
            default:
                ...
                switch ($discount->target) {
                    ...
                    case 'specific-product':
                        ...
                        $description .= ' ' . __('for product') . ' ' . implode(', ', $products);
                        break;
                    case 'customer':
                        ...
                        $description .= ' ' . __('for customer') . ' ' . implode(', ', $customers);
                        break;
                    case 'group-products':
                        $collections = DiscountProductCollection::where('discount_id', $discount->id)
                            ->join('ec_product_collections', 'ec_product_collections.id', '=',
